...
current_year0 =  check_read_fun(fn_3_output, wk = week_filter_hx)
...
#GET DATA
1_input = check_read_fun(fn_database_update, 
                            yr = year_filter_hx, 
                            wk = week_filter_hx)
...
gsheet_pull(routine_trap_tl_key, sheet = "data", out_fn = "1_input/routine_trap_yr.csv")
routine_traps = read.csv("1_input/routine_trap_yr.csv") %>%
...
write.csv(data_zone_wk, "3_output/hx_data.csv", row.names = F)
...
write.csv(hx_vi_report, "3_output/table1b_hx_vi.csv", row.names = F)
write.csv(hx_abund_report, "3_output/table2b_hx_abund.csv", row.names = F)
write.csv(hx_pir_report, "3_output/table3b_hx_pir.csv", row.names = F)
...
ggsave("3_output/plots/hx_plot_all.png", p_hx_current0, height = 8, width = 12, units = "in")
...
ggsave("3_output/plots/hx_plot.png", p_hx_current, height = 8, width = 12, units = "in")
