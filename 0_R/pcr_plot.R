list2env(readRDS("1_input/config_params.RDS"),           envir = .GlobalEnv)
...
gsheet_pull(key = standards_key, 
            sheet = 'data',
            out_fn = "1_input/standards_input.csv")

std_database = read.csv("1_input/standards_input.csv")   

if(nrow(1_input %>% filter(sample_type == "undefined"))) {
...
write.csv(std_update, "3_output/standards_new.csv")
