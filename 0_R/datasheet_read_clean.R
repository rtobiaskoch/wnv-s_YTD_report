...
write_rds(1_input0, week_filterly_input_format_mid)


#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#-------------------- C H E C K   W E E K __________---------------------
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
if(any(1_input0$Week != week_filter)) {
...
if(all(input_data_col %in% names(1_input))==F) { # if any of the required input_data_col aren't in the input data throw an error
  
  stop(paste0("The column(s) ", setdiff(input_data_col, names(1_input)), " are missing from your datasheets")
       )
}
...
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
if(any(is.na(1_input$csu_id))){
  
  stop("you have missing sample ids in your data")
}

if(any(!str_detect(1_input$csu_id, "-"))) {
...
#-------------------- C L E A N   D A T E ------------------------------
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
if(is.character(1_input$trap_date)) {
  1_input = 1_input %>%
...
filtered_samples = 1_input %>%
  filter(week != week_filter|year != year_filter)


if(nrow(filtered_samples) > 0) {
  write.csv(filtered_samples, "2_mid/non_week_samples.csv")
...
if(nrow(unmatched_trap) > 0) {
  write.csv(unmatched_trap, "2_mid/unknown_trap_ids.csv")
  stop("There are unknown trap ids in your data, check the 2_mid/unkown_trap_ids.csv file.")
}
